...
-   **RESTful API Gateway with Gin (Go) →** The external-facing API is a high-performance REST API built with the Gin framework. It enforces a uniform interface using standard HTTP methods (GET, POST, DELETE), handles JWT-based authentication, and acts as the API Gateway by routing client requests to the appropriate internal gRPC microservices.
